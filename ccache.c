 ...
 */
static void
remember_include_file(char *path, HSTATE_T *cpp_hash)
{
#ifdef _WIN32
	DWORD attributes;
#endif
 ...
 */
static bool
process_preprocessed_file(HSTATE_T *hash, const char *path)
{
	char *data;
	char *p, *q, *end;
	size_t size;
...
get_object_name_from_cpp(struct args *args, HSTATE_T *hash)
{
	char *input_base;
	char *tmp;
 ...
 */
static void
hash_compiler(HSTATE_T *hash, struct stat *st, const char *path,
              bool allow_command)
{
 ...
 */
static void
calculate_common_hash(struct args *args, HSTATE_T *hash)
{
	struct stat st;
	char *p;
 ...
 */
static struct file_hash *
calculate_object_hash(struct args *args, HSTATE_T *hash, int direct_mode)
{
	int i;
	char *manifest_name;
	struct stat st;
...
ccache(int argc, char *argv[])
{
	...
	HSTATE_T common_hash;
	HSTATE_T direct_hash;
	HSTATE_T cpp_hash;
