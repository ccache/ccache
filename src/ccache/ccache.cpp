...
hash_profiling_related_data(const Context& ctx, Hash& hash)
{
  ...
  if (ctx.args_info.profile_generate) {
    ...
    if (!ctx.args_info.coverage_compilation_dir.empty()) {
      ...
      size_t sep_pos = ctx.args_info.coverage_prefix_map.find('=');
      if (sep_pos != std::string::npos) {
        std::string old_path =
          ctx.args_info.coverage_prefix_map.substr(0, sep_pos);
        std::string new_path =
          ctx.args_info.coverage_prefix_map.substr(sep_pos + 1);
