...
class ParseError
{
  ...
  template<class T>
  friend auto parse(std::string_view borrowed) noexcept
    -> tl::expected<T, json::ParseError>;
...
parse(std::string_view borrowed) noexcept -> tl::expected<T, json::ParseError>
{
  static constexpr glz::opts ParserOpts = glz::opts{
    ...
    .partial_read_nested = true,
  };
  T dep_file{};
  if (auto&& err = glz::read<ParserOpts, T>(dep_file, borrowed)) {
