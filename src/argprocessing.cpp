...
struct ArgumentProcessingState
{
  ...
  bool found_end_of_options = false;
            ...
            ArgumentProcessingState& state)
{
  ...
  if (!state.found_end_of_options) {
    if (args[i] == "--") {
      state.found_end_of_options = true;
      return nullopt;
    }
    ...
    // no need to cache them, they can in practice be ignored. All that is
    // needed is to correctly depend also on module.modulemap files, and those
    // are included only in depend mode (preprocessed output does not list
    // them). Still, not including the modules themselves in the hash could
    // possibly result in an object file that would be different from the actual
    // compilation (even though it should be compatible), so require a
    // sloppiness flag.
    if (args[i] == "-fmodules") {
    ...
    if (util::starts_with(args[i], "-x")) {
      if (args[i].length() >= 3 && !islower(args[i][2])) {
        // -xCODE (where CODE can be e.g. Host or CORE-AVX2, always starting
        // with an uppercase letter) is an ordinary Intel compiler option, not a
      ...
      // Special handling for -x: remember the last specified language before
      // the input file and strip all -x options from the arguments.
      if (args[i].length() == 2) {
        if (i == args.size() - 1) {
          LOG("Missing argument to {}", args[i]);
          return Statistic::bad_compiler_arguments;
    ...
    if (config.compiler_type() == CompilerType::gcc
      ...
      // Special case: If a GCC compiler gets -f(no-)color-diagnostics we'll
    ...
    if (config.sloppiness().is_enabled(core::Sloppy::clang_index_store)
      ...
      // a UUID that might lead to cache misses, especially when cache is shared
    ...
    if (compopt_takes_path(args[i])) {
      ...
      const size_t next =
        args[i + 1] == "-Xclang" && i + 2 < args.size() ? 2 : 1;
      ...
      auto& dest_args = compopt_affects_cpp_output(args[i]) ? state.cpp_args
                                                            : state.common_args;
      ...
      if (path_pos) {
        ...
        if (compopt_takes_concat_arg(option) && compopt_takes_path(option)) {
          const auto relpath = Util::make_relative_path(
            ctx, string_view(args[i]).substr(*path_pos));
