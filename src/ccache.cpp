...
{
  bool detect_incbin =
    !ctx.config.sloppiness().is_enabled(core::Sloppy::incbin);
                     ...
                     const std::string& deps_path)
{
  ...
  if (!hash_depfiles(ctx, nullptr, &hash, deps_path)) {
    LOG_RAW("Failure while hashing files");
    return nonstd::make_unexpected(Statistic::internal_error);
  }
         ...
         Hash* depend_mode_hash)
{
  ...
  if (ctx.config.unify_mode()
    ...
    if (se.size() != 0) {
      ...
      if (!ctx.config.run_second_cpp()) {
        return nonstd::make_unexpected(Statistic::compiler_produced_stderr);
      }
...
get_result_key_from_cpp(Context& ctx, Args& args, Hash& hash)
{
  ...
  if (ctx.args_info.direct_i_file) {
    ...
    // Use -fminimize-whitespace if available. Outside unify mode, this will
  ...
  if (ctx.config.unify_mode()) {
    TRY(process_unified_file(ctx, hash, stdout_path, deps_path));
  } else {
...
do_cache_compilation(Context& ctx, const char* const* argv)
{
  ...
  // In depend and unify mode, extend the direct hash.
