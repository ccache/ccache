...
int memccached_init(const char *conf) {
...
const char* MEMCACHED_CONF = NULL;

void backend_memccached_done(void) {
  memccached_release();
}

void backend_memccached_init(void* conf) {
  MEMCACHED_CONF = conf;
}

int backend_memcached_from_cache(const char* id, backend_load *load) {
  if (memc == NULL) {
    if (memccached_init(MEMCACHED_CONF) == -1) {
      return -2;
    }
  }

  void *cache = memccached_get(id, &load->data_obj, &load->data_stderr, &load->data_dia, &load->data_dep,
                             &load->size_obj, &load->size_stderr, &load->size_dia, &load->size_dep);
  if (!cache) {
    return -3;
  }
...
int backend_memcached_to_cache(const char* id, backend_load *load) {
  ...
  return memccached_set(id, load->data_obj, load->data_stderr, load->data_dia, load->data_dep, load->size_obj, load->size_stderr, load->size_dia, load->size_dep);
}
...
int backend_memcached_from_cache_string(const char* id, char** string, size_t* size) {
  ...
  void *cache = memccached_raw_get(id, string, size);
  if (!cache) {
    return -3;
  }
...
int backend_memcached_to_cache_string(const char* id, char* string, size_t size) {
  ...
  return memccached_raw_set(id, string, size);
}
