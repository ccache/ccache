...
- Fixed handling of extensionless input files passed with `-x assembler`
  (regression in 4.13). +
  [small]#_[contributed by Joel Rosdahl]_#
...
- Improved storage helper lookup order to prefer libexec directories over
  `PATH`. +
  [small]#_[contributed by Joel Rosdahl]_#

- Improved reliability of finding the ccache executable path by using
  OS-specific APIs instead of relying on `argv[0]`, which is not guaranteed to
  reflect the actual executable location. +
  [small]#_[contributed by Joel Rosdahl]_#

- Improved handling of `-fstack-usage` and `-fcallgraph-info` output files when
  LTO is used: ccache now correctly skips looking for `.su` and `.ci` files that
  are not generated during compilation with LTO. +
  [small]#_[contributed by Kristian Sloth Lauszus]_#


=== Build fixes

- Fixed build on systems that have `utimes(2)` but not `utimensat(2)`. +
  [small]#_[contributed by Joel Rosdahl]_#

- Fixed `mktemp(1)` calls in build scripts to pass a template argument for
  portability to macOS/BSD. +
  [small]#_[contributed by Joel Rosdahl]_#
...
- Added the `read_only_dirs` (`CCACHE_READONLY_DIRS`) config option for
  specifying one or more read-only fallback cache directories. Results not found
  in the local cache are looked up in the read-only directories and promoted
  into the local cache on a hit. This is useful in CI environments for combining
  an ephemeral writable cache with pre-populated caches. +
  [small]#_[contributed by Thorsten Klein]_#
