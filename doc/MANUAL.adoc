...
This backend stores data using a http://rpclib.net[rpclib] "rpc-server".

The server is configured with one of the *other* ccache storage backends:

-------------------------------------------------------------------------------
CCACHE_REMOTE_ONLY=true CCACHE_REMOTE_STORAGE=file://var/tmp/storage rpc-server
-------------------------------------------------------------------------------

It can keep connections open, to one of the more expensive storage backends.

Examples:

* `+rpc://localhost+`
* `+rpc://p4ssw0rd@cache.example.com:8080|connect-timeout=50+`

Optional attributes:

* *connect-timeout*: Timeout (in ms) for network connection. The default is 100.
* *operation-timeout*: Timeout (in ms) for RPC connections. The default is 10000.
