...
[[config_compiler_type_clang_normalize_whitespace]] *clang_normalize_whitespace*::
    Clang starting at version 13 supports the *-fnormalize-whitespace*
    option. The switch removes as much whitespace between tokens as
    possible which reduces the to chance of hashes of preprocessed
    output being different because of formatting changes. It is most
    useful in _<<_the_unify_mode,unify mode>>_ where the *-P* switch
...
[[config_preprocessor]] *preprocessor* (*CCACHE_PREPROCESSOR*)::

    This option can be used to force a different preprocessor than the
    compiler. Can be useful in <<_the_unify_mode,unify mode>> when the
    compiler itself does not support *-fnormalize-whitespace*. That is,
...
preprocessor output is passed to the compiler with unpredictable
consequences if preprocessor and compiler are different. For instance,
some system libraries are specific to the compiler and macros such as
`__clang_major__` are expanded by the preprocessor.
    
[[config_preprocessor_check]] *preprocessor_check* (*CCACHE_PREPROCESSORCHECK*)::

   Analog of _<<config_compiler_check,*compiler_check*>>_ for the
   <<config_preprocessor,*preprocessor*>> if defined.

[[config_preprocessor_type]] *preprocessor_type* (*CCACHE_PREPROCESSORTYPE*, see <<config_compiler_type,*compiler_type*>> for possible values)::

    Like <<config_compiler_type,*compiler_type*>> but for the
    preprocessor. Used primarily to determine whether the
    <<config_compiler_type_clang_normalize_whitespace,*-fnormalize-whitespace*>>
    option can be used.
...
[[config_sloppy_incbin]] *incbin*::
    Usually in <<_the_preprocessor_mode,preprocessor mode>> and
    <<_the_unify_mode,unify mode>>, ccache scans the preprocessed file
    for assebmler
    ...
    preprocessed output nor is the dependency information inserted into
    the *.d* generated by *-MD*/*-MMD*. Since nearly all build system
    also rely on this information, they would also be oblivious if the
    incbin file changes.
+
The incbin sloppiness option allows ccache to skip scanning for
the .incbin directive. This is mostly beneficial in unify mode where
searching for *.incbin* is the only reason for it to parse the
preprocessed file and therefore could be skipped.
+
Neither <<_the_direct_mode,direct mode>> nor
<<_the_depend_mode,depend mode>> mode will look for *.incbin*
directives, nor are
...
*time_macros*::
    ...
    source code.
[[config_sloppy_unify_with_debug]] *unify_with_debug*::
    By default debug info (*-g* compiler option) will implicitly disable
    <<_the_unify_mode,unify mode>> and fall back to
    <<_the_preprocessor_mode,preprocessor mode>> mode. With this
...
[[config_sloppy_unify_with_output]] *unify_with_output*::
    By default, <<_the_unify_mode,unify mode>> will not cache
    ...
    locations changed.
+
If _<<config_run_second_cpp,run_second_cpp>>_ is disabled, the
...
[[config_unify_mode]] *unify_mode* (*CCACHE_UNIFY* or *CCACHE_NOUNIFY*, see _<<_boolean_values,Boolean values>>_ above)::
  
    If true, the unift mode will be used. The default is true. See
    <<_the_unify_mode,The unify mode>>.
...
<<config_preprocessor_check,*preprocessor_check*>>
(*CCACHE_PREPROCESSORCHECK*) failed.
...
| output to stderr |
The compiler emitted diagnostics in <<_the_unify_mode,unify mode>> with
<<config_run_second_cpp,run_second_cpp>> disabled. Since the diagnostic
will refer to to the temporary file, it has to re-run with he original
input file.
...
the has lookup up, but uses *-P* option which suppresses line
...
only comments depends on the compiler. gcc removes any empty line in the
output while Clang only removes empty lines if there are more than 5
consecutive lines. Note that in any preprocessor mode (including unify
mode), column information (*-gcolumn-info*, enabled in gcc by default)
is never guaranteed to match.
...
* The cache hit rate will be higher and ignore certain categories of
  ...
  *-fnormalize-whitespace*).

* The preprocessed file will be much smaller and therefore takes less
  time to hash and to compile (with
  <<config_run_second_cpp,run_second_cpp>> off).

Disadvantages:

* Changes that only change line numbers (for instance, adding or
  removing an empty line) do not trigger a recompilation. This would
  cause line numbers after the change to not match with the cached
  version. However, unify mode is implicitly disabled when compiling
  with debug info or if the compiler emits diagnostics unless the
  corresponding sloppiness option is used
  (<<config_sloppy_unify_with_debug,*unify_with_debug*>>,
  respectively
  <<config_sloppy_unify_with_output,*unify_with_output*>>).
...
*-Wno-builtin-macro-redefined -D__LINE__=* can be used to avoid this. 
