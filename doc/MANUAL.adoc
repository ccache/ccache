...
[[config_inode_cache]] *inode_cache* (*CCACHE_INODECACHE*)::
...
[[config_inode_cache_file]] *inode_cache_file* (*CCACHE_INODECACHEFILE*)::

    Specifies an optional location for the persistent inode cache file. If not
    specified the cache will be stored in *_<ccachedir>_/inode/inode_cache*.
+
It is usually not necessary to specify a nondefault location unless
*_<ccachedir>_* is located on a filesystem type that doesn't support mmap. The
cache file should always be located on a local filesystem even if
*_<ccachedir>_* is located on a remote filesystem shared by several hosts.
+
It is perfectly fine to place the cache file on tmpfs as it will be silently
regenerated if missing after a reboot.
