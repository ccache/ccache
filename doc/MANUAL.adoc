...
[[config_inode_cache_file]] *inode_cache_file* (*CCACHE_INODECACHEFILE*)::
    ...
    specified, the cache will be stored in *_<ccachedir>_/inode_cache.v$n*.
+
A version suffix in the form *.v$n*, where *$n* is the current file version,
will be appended to the given filename and should not be included in the
specified value.
+
The location must be on a local filesystem of a type that supports mmap. It is
perfectly fine to place the cache file on tmpfs as it will be silently
