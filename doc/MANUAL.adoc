...
[#config_compiler_type_clang_minimize_whitespace] 
...
[#config_preprocessor]
*preprocessor* (*CCACHE_PREPROCESSOR*)::

    This option can be used to force a different preprocessor than the
    compiler. Can be useful in <<the_unify_mode,unify mode>> when the
    compiler itself does not support *-fminimize-whitespace*. That is,
    this preprocessor is used to normalize non-significant changes while
    the compiler is used if it has changed.
+
WARNING: Compiler and preprocessor may differ in their supported compiler
options, predefined macros and (system) header search paths and therefore
the preprocessed output might be different from what the compiler may
have produced. As long as the preprocessed output is only used for hashing
effectively the only caveat is that this determines whether a recompilation
is necessary if using *preprocessor* as compiler, for instance it may not
recompile if some source code in ``#if \\__GNUC__ > 4`` has changed but
using clang as preprocessor. This is the same caveat as with
<<config_compiler_check,*compiler_check*>> or the
<<config_sloppy_system_headers,*system_headers*>> sloppiness option.
Also keep in mind that the preprocessor migh not understand all options
passed to the compiler that are usually ignored by the preprocessor,
such as `-mllvm`.
+
CAUTION: If <<config_run_second_cpp,*run_second_cpp*>> is disabled,
instead of using the preprocessor output just for hashing, 
it is also passed to the compiler with unpredictable consequences if
preprocessor and compiler are different. For instance, some system
libraries are specific to the compiler and macros such as
``\\__GNUC__`` are expanded by preprocessor predefines.

[#config_preprocessor_check]
*preprocessor_check* (*CCACHE_PREPROCESSORCHECK*)::

    Like <<config_compiler_check,*config_compiler_check*>> but for the
    preprocessor.

[[config_preprocessor_type]]
*preprocessor_type* (*CCACHE_PREPROCESSORTYPE*, see <<config_compiler_type,*compiler_type*>> for possible values)::

    Like <<config_compiler_type,*compiler_type*>> but for the
    preprocessor. Primarily used to determine whether the
    <<config_compiler_type_clang_minimize_whitespace,*-fminimize-whitespace*>>
    option can be used.
...
[#config_sloppy_incbin]
*incbin*::
    Usually, in <<the_preprocessor_mode,preprocessor mode>> and
    <<the_unify_mode,unify mode>>, ccache scans the preprocessed file
    for assambler
    https://sourceware.org/binutils/docs/as/Incbin.html[*.incbin*]
    directives (e.g. in *asm* inline assembly statements). The effect of
    the directive is to emit the referenced file without any processing
    into the object file and is sometimes used to more efficiently
    include a binary file more efficiently than by storing it in a C/C++
    constant array. However, the inclusion is not done by the
    preprocessor and therefore its content not inserted into the
    preprocessed output nor in the dependency information inserted into
    the *.d* generated by *-MD*/*-MMD* (Used in
    <<the_unify_mode,unify mode>>, <<the_direct_mode,*direct_mode*>> and
    nearly every buildsystem).
+
The incbin sloppiness option allows ccache to skip scanning for
the .incbin directive. This is mostly beneficial in
<<the_unify_mode,unify mode>> where searching for *.incbin* is the only
reason for it to parse the preprocessed file and therefore could be skipped.
...
*time_macros*::
    ...
    source code.
[#config_sloppy_unify_with_debug]
*unify_with_debug*::
    By default debug info (*-g* compiler option) will implicitly disable
    <<the_unify_mode,unify mode>> and fall back to
    <<the_preprocessor_mode,preprocessor mode>> mode. With this
    sloppiness option unify mode can still be used with when emitting
    debug. This may cause cache hits with compiler output where only
    line information has changed. A possible consequence is that
    debuggers show the wrong line number or cannot set breakpoint at
    specific line numbers.
+
If <<config_run_second_cpp,run_second_cpp>> is disabled, the debug
info will instead point to the file containing the preprocessed source.
The file is temporary and will have been deleted already which makes the
debug info rather useless.

[#config_sloppy_unify_with_diagnostics]
*unify_with_diagnostics*::
    By default, <<the_unify_mode,unify mode>> will not cache
    preprocessed results if there was any compiler output. Such output
    usually contains line numbers that, like with debug info, may be off
    a few lines if there was a cache hit with a file where only line
    locations changed.
+
If <<config_run_second_cpp,run_second_cpp>> is disabled, the
compiler will instead emit line diagnostics that refer to the temporary
file containing the preprocessed file.
--
+
...
[#config_unify_mode]
*unify_mode* (*CCACHE_UNIFY* or *CCACHE_NOUNIFY*, see _<<Boolean values>>_ above)::
  
    If true, the unify mode will be used. The default is true. See
    <<the_unify_mode,The unify mode>>.
...
* the *unify mode*, where ccache additionally does not try to preserve
  line breaks such that formatting changes do not trigger recompilation
...
Unify mode can be seen as a combination of depend and preprocessor mode.
Like preprocessor mode, the preprocessed intermediate file is used for
the hash lookup up, but uses *-P* option which suppresses line
information in the preprocessed file. As a consequence, like in depend
mode, ccache uses the compiler's generated dependency files to track
changes in which files have been included. The following flow chart
illustrates how unify mode works.

image:unify_mode.svg[Unify mode flow chart]

Unify mode is most powerful when used in conjunction with Clang's
<<config_compiler_type_clang_minimize_whitespace,*-fminimize-whitespace*>>.
This flag normalizes all line line breaks unless semantically required
(e.g. after a #pragma or inside a raw string). Otherwise, with *-P*
only, the preprocesser still keeps those tokens on the same line how
they are in the input file. The handling of empty lines of lines with
only comments depends on the compiler. Note that in any preprocessor
mode (including unify mode), column information (*-gcolumn-info*,
enabled in gcc by default) is never guaranteed to match.

Advantages:

* The cache hit rate will be higher and ignore certain categories of
  file changes such as adding/removing comments or whitespace
  reformatting (such as most changes done by clang-format if using
  *-fminimize-whitespace*).

* The preprocessed file will be smaller and therefore takes less
  time to hash to process.

Disadvantages:

* Line numbers are relevant in some situations, such as for debug
  information and compiler diagnostics (warnings/errors). Shifted line
  numbers (e.g. adding/removing empty lines) alone would does not
  trigger a compilation, hence unify mode is implicitly disabled when
  compiling with debug info (*-g* flag) or if the compiler emits
  diagnostic output. Whether the compiler emits diagnostics can only be
  determined after the compiler has already run and subsequent
  compilations can only cache-hit in direct mode. However, most coding
  guidelines seek a warning-free compilation anyway. This behavior can
  be overridden using the <<config_sloppy_unify_with_debug,*unify_with_debug*>>
  or <<config_sloppy_unify_with_diagnostics,*unify_with_diagnostics*>> sloppiness
  options.

* The mode is new and not as well tested.

The `\\__LINE__` macros are replaced with the actual lines by the
preprocessor and therefore will still cause a recompilation if there
are line-only changes before the `\\__LINE__` macro in the same file.
`\\__LINE__` is commonly used in tracing libraries and the `assert` macro.
As a result, an *assert* in an header with a whitespace-only change can
still trigger the recompilation of all source files that include it.
If the actual location is unimportant,
`-Wno-builtin-macro-redefined -D\\__LINE__=` can be used to avoid this. 
This is non-standard but works in Clang and gcc.

If you know that the `.incbin` directive is not used, it is recommended
to use the <<config_sloppy_incbin,*incbin*>> sloppiness option.

WARNING: While <<config_run_second_cpp,*run_second_cpp*>> works as
documented, combining it with unify mode is not recommended. Together
with removing line information from the preprocessed output, it also
does not preserve which declarations are from a system header. Compiler
behaviour may differ in system headers, in particular compiler
diagnostics are suppressed when originating from a system header.
That is, with <<config_sloppy_unify_with_diagnostics,*unify_with_diagnostics*>>
sloppiness option, this results in noisy compiler diagnostics.
Without <<config_sloppy_unify_with_diagnostics,*unify_with_diagnostics*>>, any
compiler output will trigger a fallback compiler invocation using the
original files and without caching.
 ...
 * Neither <<_the_direct_mode,direct mode>> nor <<the_depend_mode,depend mode>>
   take *.incbin* directives into account, nor does any mode handle
   https://sourceware.org/binutils/docs/as/Include.html#Include[*.include*]
  directives.
