...
[[config_inode_cache_file]] *inode_cache_file* (*CCACHE_INODECACHEFILE*)::
    ...
    specified, the cache will be stored in
    */run/user/$(id -u)/ccache_inode_cache.v$n* if the directory
    */run/user/$(id -u)* exists, otherwise in */tmp/ccache_inode_cache.v$n*.
+
A version suffix in the form *.v$n*, where *$n* is the current file version,
will always be appended to the given filename and should not be included in the
specified value.
