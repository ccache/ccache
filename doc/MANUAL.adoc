...
*clang_modules*::
    ...
    it cannot hash the state of Clang's internal representation of relevant
    modules. This sloppiness allows caching in such a case. See
    _<<Clang modules>>_ for more information.
*pch_defines*::
...
Clang additionally supports an alternative non-standard form of modules which
are different from and pre-date C++20 standard modules. This non-standard form
of modules is not limited to C++ but supports other C-family languages.

This alternative form of modules is enabled for Clang in the following scenarios:

1. When the `-fmodules` option is used without `-std=c++20` or newer
2. When `module.modulemap` files are used

For compilation of these non-standard modules, ccache in practice only hashes
the `module.modulemap` files and no additional data. This is because there isn't
enough information exposed about this compilation model by Clang for ccache to
comprehensively hash and cache all the extra structures involved.

This limitation should not matter in practice: as long as everything else
(including `module.modulemap` files) is the same the cached result should work.

Still, you must set <<config_sloppiness,*sloppiness*>> to *clang_modules* to
enable the caching.

You must also use both <<The direct mode,*direct mode*>> and
<<The depend mode,*depend mode*>>.

When using <<The preprocessor mode,the preprocessor mode>> Clang does not
provide enough information even to allow hashing of `module.modulemap` files.
...
* If "`Could not use Clang modules`" has been incremented, see _<<Clang modules>>_.
