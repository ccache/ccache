...
- Add support for clang option `-MJ` which generates a compile_commands.json
  fragment if sloppiness "compile_command" is set.
  Also add explicit `--ccache-MJ output.ccmd` which writes to the provided file
  and `--ccache-auto-MJ` which automatically deduces the output.ccmd filename
  from the output object filename. These last two arguments do not require any
  sloppiness to be set.
