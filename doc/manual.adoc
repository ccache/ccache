...
+
The paths that the compiler writes into the dependency file, and into the file
produced by MSVC's `/sourceDependencies` option, are made relative in the same
way. A program that reads one of those files from another directory therefore
has to resolve the relative paths itself.
+
