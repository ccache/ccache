...
`-fprebuilt-implicit-modules` is not supported and such compilations are
counted as "`Could not use modules`". Clang looks up implicit modules in a
subdirectory layout named by hashes of compiler internals, so ccache can't tell
which files the compilation reads.
