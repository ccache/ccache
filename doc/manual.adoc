...
* the contents of GCC specs files: those given with `-specs` (including files
  they pull in with `%include`), the implicit `specs` file GCC finds via `-B`,
  `GCC_EXEC_PREFIX` or `LIBRARY_PATH`, and one in GCC's installation directory
