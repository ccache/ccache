...
*--trim-marker* _PATH_::
    ...
    Speed up repeated `--trim-recompress` runs by skipping files whose
    modification time is older than the marker file at _PATH_.
...
*--trim-recompress* _LEVEL_::
    ...
    long time since all files in the cache need to be visited. Only files that
    are currently compressed with a different level than _LEVEL_ will be
    recompressed. To avoid revisiting unchanged files on subsequent runs, see
    `--trim-marker`.
