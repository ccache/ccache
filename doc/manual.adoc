...
*modules*::
    ...
    *Use case*: Clang builds using `-fmodules` or
    `-fprebuilt-implicit-modules`. +
    *Effect*: Allows caching when C++ modules are used. +
    *Trade-off*: May not detect changes in module internal state, or in
...
* the contents of GCC specs files: those given with `-specs`, the implicit
  `specs` file GCC finds via `-B`, `GCC_EXEC_PREFIX` or `LIBRARY_PATH`, and
  those in GCC's installation directory
...
* `+__has_include+` or `+__has_include_next+` with a macro operand is present in
  the source code
* `#embed` or `+__has_embed+` is present in the source code
...
* With <<config_safe_direct_mode,*safe_direct_mode*>>, ccache also notices a
  `module.modulemap` or `module.private.modulemap` file appearing in the
  directory of an included header or one of its parent directories.
...
Regardless of the above, ccache hashes the module files that a compilation
reads, so that a consumer is recompiled when a module it imports changes. This
works for the options that tell ccache which files those are:

* `-fmodule-file=`, which gives the path directly.
* `-fprebuilt-module-path=`, whose directory Clang searches for
  `<module-name>.pcm`. The command line doesn't say which of those files the
  compilation reads, so all of them are hashed, names as well as contents. This
  costs a hit when an unrelated module file in the same directory changes.

`-fprebuilt-implicit-modules` compilations are counted as "`Could not use
modules`" unless <<config_sloppiness,*sloppiness*>> is set to `modules`. Clang
looks up implicit modules in a subdirectory layout named by hashes of compiler
internals, so ccache can't tell which files the compilation reads.
