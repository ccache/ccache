...
TEST(sysroot_should_be_rewritten_if_basedir_is_used)
{
  ...
  ctx.current_working_dir = get_cwd();
  arg_string =
    format("cc --sysroot=%s/foo/bar -c foo.c", ctx.current_working_dir);
...
TEST(sysroot_with_separate_argument_should_be_rewritten_if_basedir_is_used)
{
  ...
  ctx.config.set_base_dir(get_root());
  ctx.current_working_dir = get_cwd();
  arg_string = format("cc --sysroot %s/foo -c foo.c", ctx.current_working_dir);
  orig = args_init_from_string(arg_string);
  free(arg_string);
...
TEST(isystem_flag_with_separate_arg_should_be_rewritten_if_basedir_is_used)
{
  ...
  ctx.config.set_base_dir(get_root());
  ctx.current_working_dir = get_cwd();
  arg_string = format("cc -isystem %s/foo -c foo.c", ctx.current_working_dir);
  orig = args_init_from_string(arg_string);
  free(arg_string);
...
TEST(isystem_flag_with_concat_arg_should_be_rewritten_if_basedir_is_used)
{
  ...
  cwd = get_posix_path(ctx.current_working_dir);
