SUITE_remote_rediss_PROBE() {
    if ! $CCACHE --version | fgrep -q -- rediss-storage &> /dev/null; then
        echo "rediss-storage not available"
        return
    fi
...
SUITE_remote_rediss() {
    ...
    export CCACHE_REMOTE_STORAGE="${redis_url}|cacert=ca.crt|cert=client.crt|key=client.key"
    ...
    export CCACHE_REMOTE_STORAGE="rediss://localhost:1"
