SUITE_secondary_tkrzw_rpc_PROBE() {
...
start_tkrzw_server() {
    ...
    dbm="$(mktemp).tkh"
    tkrzw_server --address "localhost:${port}" "$dbm" >/dev/null &
    # Wait for server start.
    i=0
    while [ $i -lt 100 ] && ! tkrzw_dbm_remote_util echo --address "localhost:${port}" &>/dev/null; do
        sleep 0.1
        i=$((i + 1))
    done
}

SUITE_secondary_tkrzw_rpc_SETUP() {
    unset CCACHE_NODIRECT

    generate_code 1 test.c
}

expect_number_of_tkrzw_rpc_cache_entries() {
    local expected=$1
    local url=$2
    local actual

    address=$(echo $url | sed -e "s|^tkrzw://||")
...
SUITE_secondary_tkrzw_rpc() {
    # -------------------------------------------------------------------------
    TEST "Base case"

    port=8888
    tkrzw_url="tkrzw://localhost:${port}"
    export CCACHE_SECONDARY_STORAGE="${tkrzw_url}"

    start_tkrzw_server "${port}"

    $CCACHE_COMPILE -c test.c
    expect_stat direct_cache_hit 0
    expect_stat cache_miss 1
    expect_stat files_in_cache 2
    expect_number_of_tkrzw_rpc_cache_entries 2 "$tkrzw_url" # result + manifest
